...
namespace Constraints {
    ...
    struct Constraint {
        ...
        Constraint(uint64_t _node, Type t, size_t val) {
            ...
            if((t == BRANCH_SIZE_MAXIMUM) || (t == BRANCH_SIZE_MINIMUM) || (t == BRANCH_SIZE_EQUALS)){
                type = t;
        ...
        bool is_satisfied(const uint64_t _node, const Branch& b) const {
            ...
            switch(type){
                case BRANCH_SIZE_MAXIMUM: return not_relevant(_node) || (pts_size <= std::get<size_t>(value));
                case BRANCH_SIZE_MINIMUM: return not_relevant(_node) || (pts_size >= std::get<size_t>(value));
                case BRANCH_SIZE_EQUALS: return not_relevant(_node)  || (pts_size == std::get<size_t>(value));
